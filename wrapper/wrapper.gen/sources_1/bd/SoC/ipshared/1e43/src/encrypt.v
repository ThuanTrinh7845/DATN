...
// --- S?A L?I LOGIC ÐI?U KHI?N FW VÀ ENCODE ---
// Thay v? dùng always @(state...) t? h?p, ta ðýa h?t vào clock
always@(posedge clk)
begin
     if (rst) begin
        state <= s_wait_start;
        ...
        // RESET CÁC TÍN HI?U ÐI?U KHI?N
        start_fw <= 0;
        start_encode <= 0;
        start_fw_transfer <= 0;
    end
    else begin
        // M?c ð?nh t?t các tín hi?u start (nh?p xung)
        start_fw <= 0;
        // start_encode <= 0; // encode ch?y dài hõn nên c?n th?n
        ...
        if (state == s_wait_start) begin
            ...
            start_fw_transfer <= 0; // Reset
            
            if (start) begin
                state <= s_gen_r1;              
                start_fw <= 1'b1;     // Kích ho?t FW
                start_encode <= 1'b1; // Kích ho?t Encode
            end else begin
                start_encode <= 1'b0;
            end
        end
             ...
             r2_done <= 0;
             done_fixed_weight <= 0;
             start_encode <= 0; // T?t encode start
             
             if (done_fw) begin
                 start_fw_transfer <= 1'b1; // Kích ho?t transfer
...
// Xóa b? kh?i always t? h?p gây l?i Latch ? ðây
...
// --- S?A L?I LOGIC SEL_HS (DÙNG Ð?NG B? Ð? TRÁNH LATCH) ---
always@(posedge clk)
begin
    ...
    if (rst) begin
        ...
        sel_e <= 0;
        sel_hs <= 0; // RESET T?I ÐÂY
    end
    else begin
        // Gán m?c ð?nh ð? tránh latch (n?u có logic t? h?p) 
        // nhýng ? ðây ta dùng logic tu?n t? (sequential) nên gi? giá tr? c? là OK.
        ...
        if (v_state == v_wait_start) begin
            ...
            if (u_done) begin
                v_state <= v_s_mul_r2;
                sel_hs <= 1'b1; // Chuy?n sang ð?c s
            end
            else begin
                sel_hs <= 1'b0; // M?c ð?nh ð?c h
            end
        end
        
        else if (v_state == v_s_mul_r2) begin
            sel_hs <= 1'b1; // Gi? nguyên ð?c s
            if (done_poly_mult) begin
