...
reg [3:0] state     = 0;
  ...
  // --- FIX 1: THÊM RESET CHO GEN_ONE_REG ---
  always@(posedge clk)
  begin
    if (rst) begin
        gen_one_reg <= 0;
        ...
        gen_one_reg <= gen_one; // Dùng non-blocking (<=) cho chu?n thi?t k?
    end
  end 
  
  
  // --- FIX 2: THÊM RESET CHO LOCATION_REG ---
    always@(posedge clk)
  begin
    if (rst) begin
        location_reg <= 0;
...
assign add_out_addr = add_out_addr_reg;
  ...
  // --- FIX 3: THÊM RESET CHO ADD_OUT_ADDR_REG (QUAN TR?NG NH?T) ---
  always@(posedge clk)
  begin
    if (rst) begin
        add_out_addr_reg <= 0;
    end
    else if (pm_rd_en == 1'b1) begin
