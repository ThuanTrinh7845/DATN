...
always@(posedge clk) begin
    if (weight_counter_init)
        weight_count <= 0;
    else if (wr_en_ms)
        weight_count <= weight_count + 1;
    else if (decrease_weight_count)
        weight_count <= weight_count - 1;
end
...
// --- FSM states ---
(* mark_debug = "true" *) wire [4:0]  dbg_red_state;
(* mark_debug = "true" *) wire [4:0]  dbg_ctx_state;
(* mark_debug = "true" *) wire [3:0]  dbg_state_shake;

// --- Rejection pipeline ---
(* mark_debug = "true" *) wire [23:0] dbg_shake_ctx_out;     // input to barrett
(* mark_debug = "true" *) wire [M-1:0] dbg_data_in_ms;       // output of barrett (= reduced value)
(* mark_debug = "true" *) wire        dbg_rejection_pass;    // rejection threshold check
(* mark_debug = "true" *) wire        dbg_wr_en_ms;          // write enable (before pipeline)
(* mark_debug = "true" *) wire        dbg_wr_en_ms_reg_0;    // pipeline stage 1
(* mark_debug = "true" *) wire        dbg_wr_en_ms_reg_1;    // pipeline stage 2 (actual write)

// --- Address tracking ---
(* mark_debug = "true" *) wire [LOG_WEIGHT:0] dbg_wr_addr_ms;       // write address (before pipeline)
(* mark_debug = "true" *) wire [LOG_WEIGHT-1:0] dbg_wr_addr_ms_reg_1; // actual write address
(* mark_debug = "true" *) wire [LOG_W_CTX:0] dbg_rd_addr_ctx;       // read address for shake context
(* mark_debug = "true" *) wire [LOG_W_CTX:0] dbg_count_red;

// --- Weight tracking ---
(* mark_debug = "true" *) wire [LOG_WEIGHT:0] dbg_weight_count;
(* mark_debug = "true" *) wire        dbg_weight_counter_init;
(* mark_debug = "true" *) wire        dbg_squeeze_ctrl;

// connect
assign dbg_red_state        = red_state;
assign dbg_ctx_state        = ctx_state;
assign dbg_state_shake      = state_shake;
assign dbg_shake_ctx_out    = shake_ctx_out;
assign dbg_data_in_ms       = data_in_ms;
assign dbg_rejection_pass   = rejection_threshold_pass;
assign dbg_wr_en_ms         = wr_en_ms;
assign dbg_wr_en_ms_reg_0   = wr_en_ms_reg_0;
assign dbg_wr_en_ms_reg_1   = wr_en_ms_reg_1;
assign dbg_wr_addr_ms       = wr_addr_ms;
assign dbg_wr_addr_ms_reg_1 = wr_addr_ms_reg_1;
assign dbg_rd_addr_ctx      = rd_addr_ctx;
assign dbg_count_red        = count_red;
assign dbg_weight_count     = weight_count;
assign dbg_weight_counter_init = weight_counter_init;
assign dbg_squeeze_ctrl     = squeeze_ctrl;
// end ILA
endmodule